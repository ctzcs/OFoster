...
VirtualAxisUpdate :: proc(v: ^VirtualAxis, t: runtime.Time) { _ = t; if v.Base.IsDisposed || v.Base.Input == nil { return }; v.Value=sets.AxisBindingSetValue(&v.Set,v.Base.Input,v.Base.ControllerIndex); if v.Value > 0 { v.IntValue=1 } else if v.Value < 0 { v.IntValue=-1 } else { v.IntValue=0 }; v.PressedSign=sets.AxisBindingSetPressedSign(&v.Set,v.Base.Input,v.Base.ControllerIndex) }
